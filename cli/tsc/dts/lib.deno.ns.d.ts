...
declare namespace Deno {
  ...
  export class UnsafePointerView {
     ...
     *
     * Invalid UTF-8 causes U+FFFD character(s) in returned string.
     *
     * Similar to:
     * ```ts
     * function getCString(this: Deno.UnsafePointerView, offset: number = 0) {
     *   let byteLength = 0;
     *   while (this.getUint8(offset + byteLength)) {
     *     ++byteLength;
     *   }
     *   return new TextDecoder().decode(this.getArrayBuffer(byteLength, offset));
     * }
     * ``` */
     ...
     * Similar to:
     * ```ts
     * function getCString(pointer: Deno.PointerObject, offset: number = 0) {
     *   const view = new Deno.UnsafePointerView(pointer);
     *   let byteLength = 0;
     *   while (view.getUint8(offset + byteLength)) {
     *     ++byteLength;
     *   }
     *   return new TextDecoder().decode(view.getArrayBuffer(byteLength, offset));
     * }
     * ``` */
